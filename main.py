...
class MainApp(MDApp):
    ...
    def toggle_theme_style(self, switch_instance, is_active):
        self.theme_cls.theme_style = "Dark" if is_active else "Light"
        theme_label = self.manager.get_screen('settings').ids.theme_label
        theme_label.text = "Dark Mode" if is_active else "Light Mode"
    ...
    def run_code(self):
        code_input = self.manager.get_screen('editor').ids.code_input
        console_output = self.manager.get_screen('editor').ids.console_output
        ...
        console_output.text = redirected_output.getvalue()
...
class ScreenEditor(Screen):
    def on_pre_enter(self, *args):
        code_input = self.ids.code_input
        code_input.lexer = PythonLexer()
